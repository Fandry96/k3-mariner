...
## 2025-03-01 - Persistent DDGS Connection Pool Optimization
**Learning:** Instantiating a new `duckduckgo_search.DDGS()` context for every search destroys the underlying HTTP connection pool, requiring fresh DNS, TCP, and TLS handshakes, which significantly slows down un-cached searches.
**Action:** Always reuse a persistent `DDGS()` instance (e.g., via `@st.cache_resource` in Streamlit) to maintain the connection pool and reduce search latency.
