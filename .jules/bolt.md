...
**Learning:** Using `@functools.lru_cache` on an instance method like `forward` in a Tool class causes memory leaks because it caches `self`, keeping the entire instance alive.
**Action:** Instead, implement a bounded instance-level dictionary (`self._cache = {}`) inside the tool itself to safely cache results without leaking memory for long-lived instances.
