...
## 2025-03-01 - Agent Tool Instance Caching and Generator Expressions
**Learning:** Agent tool invocations frequently repeat identical external queries, wasting time and API quotas. Additionally, string formatting loops using list comprehensions (`[... for ...]`) can unnecessarily inflate peak memory footprint compared to generator expressions (`(... for ...)`), especially within constrained agent contexts.
**Action:** Implement bounded instance-level dictionary caching (`self._cache = {}`) within Agent `Tool` classes to prevent redundant external API queries while strictly limiting cache size to prevent slow memory leaks for long-lived agent instances. Transition list comprehensions to generator expressions within `join()` formatting calls to reduce peak memory allocation.
