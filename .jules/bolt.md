...
## 2025-04-23 - Bounded Tool Caching Edge Case
**Learning:** When using a bounded instance-level dictionary cache (e.g., `self._cache = {}`) for agent tools, enforcing the bounds without assigning to a local variable first can trigger a `KeyError` if the cache was just cleared.
**Action:** Always assign the computed result to a local variable first, update the cache, enforce bounds (`if len > 50: clear()`), and then return the local variable.
