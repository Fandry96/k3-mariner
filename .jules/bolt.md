...
## 2025-05-25 - Multi-Layer Agent Tool Caching
**Learning:** For agent tools used inside Streamlit applications, caching at the Streamlit API layer (e.g., `@st.cache_data`) alone still incurs framework overhead during repeated queries within a single agent execution.
**Action:** Implement multi-layer caching by adding a bounded instance-level dictionary (`self._cache = {}`) within the tool class to bypass Streamlit API overhead for repeated queries, ensuring errors are not cached to prevent cache poisoning.
