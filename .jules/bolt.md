...
## 2024-05-24 - [Bounded Instance Caching for Agent Tools]
**Learning:** When using instance-level dictionaries for caching agent tool outputs, we must enforce an LRU-style bounded size limit and assign computed values to variables before eviction checks to prevent `KeyError` exceptions and unbounded memory growth.
**Action:** Always implement a bounded size limit (e.g., 50 items) and intentional error-state skipping for instance-level caches in long-running agent components.
