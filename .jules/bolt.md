...
## 2025-03-09 - LRU Caching for Tool Outputs
**Learning:** Returning early without caching on empty API results (like 'No results found.') causes repeated identical queries to bypass the cache and trigger expensive network calls.
**Action:** When caching tool outputs, ensure that valid 'empty' responses are appropriately formatted and cached to maintain performance under rate-limiting, while skipping actual exception states.
