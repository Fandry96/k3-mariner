...
## 2025-03-02 - Tool Output Caching for Agents
**Learning:** Agents often repeatedly call search tools with identical queries during iterative planning. Without an instance-level cache, this triggers expensive, redundant network calls. Furthermore, returning early on "no results" without caching causes the agent to re-query the same failing term.
**Action:** Implement bounded, instance-level LRU caching in agent tools using dictionary pop/re-insert to maintain recency. Always cache valid empty states (like 'No results found.') but never cache actual exception/error states.
