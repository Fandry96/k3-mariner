...
**Learning:** Agent execution often involves repeated tool calls with the same arguments. Relying solely on `@st.cache_data` incurs Streamlit API overhead during tight agent loops.
**Action:** Implement multi-layer caching: use `@st.cache_data` for cross-rerun global persistence, and an instance-level bounded dictionary (`self._cache = {}`) inside the tool class to instantly serve repeated queries within a single agent run.
