...
## 2025-03-01 - Instance Caching for Agent Tools
**Learning:** Relying solely on `@st.cache_data` adds overhead for repeated queries within the same tool execution run. Wait to cache the final output instead of individual raw objects.
**Action:** Used an instance-level dictionary cache (`self._cache = {}`) inside the tool class to bypass external caching limits and added LRU eviction bounds.
