...
## 2025-05-10 - Bounded Instance Caching for Tools
**Learning:** Using `@functools.lru_cache` on class methods causes memory leaks by holding `self` references, and global caches like `@st.cache_data` add unnecessary API overhead for in-session repeated tool calls.
**Action:** Implement bounded instance-level dictionary caching (`self._cache`) with manual eviction (`if len > N: pop`) for agent tools, assigning the computed result to a local variable before updating the cache to prevent `KeyError`.
