...
## 2025-03-02 - Generator Expressions in join
**Learning:** Using a list comprehension inside `join()` unnecessarily allocates memory for the entire intermediate list before joining it.
**Action:** Always replace list comprehensions inside `.join()` calls with generator expressions to avoid unnecessary intermediate memory allocations and improve CPU cache hits.

## 2025-03-02 - DuckDuckGo Search Client Caching
**Learning:** Instantiating the DDGS client inside an `lru_cache` wrapped function destroys underlying HTTP connection pooling (TCP/TLS reuse) for each unique call.
**Action:** Cache a method on a class instance that reuses a single established client, ensuring valid 'empty' responses are appropriately formatted and cached while errors skip caching.
