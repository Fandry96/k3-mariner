...
## 2025-03-09 - Caching Tool Outputs and Empty States
**Learning:** Returning early without caching on valid 'empty' responses causes repeated identical queries to bypass the cache and continuously trigger expensive network calls.
**Action:** When implementing LRU caching for network tool outputs, always properly format and cache valid empty responses, while explicitly skipping caching for exception or error states.
