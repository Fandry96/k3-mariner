...
## 2025-03-08 - Tool Output Instance LRU Caching
**Learning:** DuckDuckGo web searches can be heavily duplicated by autonomous agents. Standard Python dictionaries preserve insertion order, which can be leveraged for bounded LRU caching by popping and re-inserting accessed keys.
**Action:** Implement instance-level bounded LRU caches (e.g., `self._cache`) for expensive tool operations, ensuring error states bypass the cache while valid empty states (e.g., 'No results found.') are cached to prevent repeated failure loops.
