...
## 2025-05-24 - Tool Output Caching
**Learning:** For expensive tool outputs like web searches, repeated identical queries cause redundant network calls. Returning early without caching on empty results causes them to bypass the cache and continuously trigger expensive calls.
**Action:** Implement bounded LRU caching for tool outputs, ensuring that valid empty results are appropriately formatted and cached, while skipping caching for exception or error states.
