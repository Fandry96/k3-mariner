...
## 2025-03-01 - Tool Search Caching Strategy
**Learning:** Returning early without caching on empty results ("No results found.") causes repeated identical empty queries to bypass the cache and continuously trigger expensive network calls.
**Action:** When caching API results, ensure that valid 'empty' responses are appropriately formatted and stored in the cache alongside successful results.
