...
## 2025-03-01 - Multi-Layer Agent Tool Caching
**Learning:** Streamlit's `@st.cache_data` has noticeable serialization/deserialization overhead. For agent tools called repeatedly in a loop, hitting the Streamlit cache on every call degrades performance.
**Action:** Implement multi-layer caching: use `@st.cache_data` on a helper function for cross-rerun global persistence, and an instance-level dictionary (`self._cache = {}`) within the tool class to bypass Streamlit API overhead for repeated queries within a single agent execution.
