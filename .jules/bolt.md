...
## 2024-07-08 - Bounded LRU caching in agent tools
**Learning:** Standard caching libraries like `@lru_cache` are stateless. When an agent object is re-created, the cache may be lost. Instance-level caching on the Tool instance using a plain dictionary maintains cache for the lifespan of that specific agent/tool process, which is optimal for short-lived research agents executing multiple identical sub-queries.
**Action:** When implementing caching on agent tools, use an instance-level dictionary (`self._cache = {}`) with manual LRU enforcement (pop and re-insert on hit, bound eviction on miss).
