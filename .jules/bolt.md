...
## 2025-03-01 - Bounded Agent Tool Caching
**Learning:** Unbounded instance-level caching causes slow memory leaks for long-lived instances processing many unique queries, while lacking a cache leads to highly redundant operations. Additionally, list comprehensions in formatting strings (like `\n.join([])`) increase peak memory usage compared to generators.
**Action:** When implementing an instance-level dictionary (`self._cache = {}`) to cache agent tool outputs, enforce a bounded size limit (e.g., `if len(self._cache) > 50: self._cache.clear()`). Assign the computed result to a local variable first before returning to prevent `KeyError` if the cache was just cleared. Use generator expressions within `str.join()` calls.
