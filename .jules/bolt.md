...
## 2025-03-02 - LRU Caching for Tool Outputs
**Learning:** In smolagents, repeated API tool calls (like DuckDuckGo search) can severely degrade agent performance and lead to rate limiting.
**Action:** Implement LRU caching directly within tool instances (`self._cache`) for expensive API calls. Enforce bounds to prevent memory leaks and explicitly perform recency updates by popping and re-inserting the accessed key on a cache hit.
