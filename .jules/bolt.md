...
## 2025-05-25 - Agent Tool Caching Optimization
**Learning:** Tools used by agents can be called with the same arguments repeatedly, resulting in redundant network calls and Streamlit API overhead. Using `@functools.lru_cache` on class methods introduces memory leaks by holding references to `self`.
**Action:** Implement bounded instance-level dictionary caching (`self._cache = {}`) inside the tool class to avoid `lru_cache` memory leaks while preventing redundant work. Ensure transient exceptions are not cached to prevent permanent cache poisoning.
