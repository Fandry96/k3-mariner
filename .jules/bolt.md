...
## 2025-05-15 - Bounded Cache for Agent Tools
**Learning:** In autonomous agents, caching tool outputs prevents duplicate network calls, but unbound caches cause memory leaks, and caching transient error states prevents recovery.
**Action:** Always implement a bounded instance-level cache (e.g., `self._cache` with LRU eviction limit) for agent tool outputs, and ensure error states are intentionally skipped from caching.
