...
## 2025-03-05 - Bounded LRU Caching for Agent Tools
**Learning:** In autonomous agents, identical tool queries (like repeated web searches) occur frequently. Without caching, this triggers redundant, slow network calls that block execution. Unbounded caching causes memory leaks. Python dictionaries maintain insertion order, enabling O(1) LRU caching by popping and re-inserting accessed keys.
**Action:** Implement bounded instance-level LRU caching for expensive tool operations (like web search) using dict pop-and-insert for recency. Ensure empty responses are cached while errors are skipped.
