...
## 2025-03-01 - Instance-Level Tool Caching
**Learning:** In long-running autonomous agent sessions, tools (like web search) are often called repeatedly with the same query due to agent loops or retries. Without instance-level caching, this results in redundant network calls, slowing down the overall task execution and risking rate limits.
**Action:** When implementing tools for agents, utilize an instance-level bounded cache dictionary (`self._cache`) with simple LRU-style eviction (e.g., limit to 50 items). Ensure exceptions/failures are explicitly bypassed during caching so the agent can correctly recover and retry later without hitting a cached error state.
