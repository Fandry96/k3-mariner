...
## 2024-07-10 - Agent Search Caching
**Learning:** In autonomous agents, caching tools locally using a bounded LRU dictionary prevents redundant network requests during recursive thinking steps.
**Action:** Always implement caching on search tools using explicit LRU bounding and dictionary updates (`val = pop(k); cache[k] = val`) to enforce recency order.
