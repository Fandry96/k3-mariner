...
## 2025-03-01 - Duplicate Network Operations
**Learning:** Tools wrapping network APIs (like DuckDuckGo search) execute a request on every invocation, even for identical queries. This redundantly hits rate limits and wastes seconds per call.
**Action:** Implement a bounded LRU cache (e.g., dictionary with pop/re-insert) at the instance level for such tools to transform identical subsequent N-time network calls into O(1) memory lookups.
