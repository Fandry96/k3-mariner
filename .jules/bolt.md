...
**Learning:** Cleaning ANSI escape codes from a growing stdout buffer using regex on the full history ($O(N^2)$) causes severe performance degradation and UI freezes for large logs.
**Action:** Implement incremental cleaning ($O(N)$) by processing only new chunks before appending to the buffer, accepting a minor risk of split ANSI codes in exchange for massive performance gains (900x speedup).
