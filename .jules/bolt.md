...
## 2024-06-20 - [Bounded Tool Output Caching]
**Learning:** Unbounded dictionary caching for agent tools can lead to unbounded memory growth and thrashing. Caching must safely handle local assignment before bounded eviction to avoid KeyErrors, and intentionally skip error states to allow retry recovery.
**Action:** Enforce an LRU-style bounded limit (e.g., 50) on `self._cache` and assign results to a local variable before updating the cache and returning it.
