...
## 2024-07-24 - LRU Caching for Agent Tool Outputs
**Learning:** Tool outputs in non-UI environments lack framework-level caching, causing redundant, expensive network calls and rate limiting on identical tool queries. Returning early on empty results without caching them triggers continuous network calls.
**Action:** Implement bounded LRU-style dictionary caching directly within the Tool instance, ensuring even 'empty' valid results are cached while skipping actual error states.
