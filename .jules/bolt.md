...
## 2025-03-05 - DDGS Connection Pool Destruction
**Learning:** Instantiating a new `duckduckgo_search.DDGS()` context for every search destroys the underlying HTTP connection pool, causing fresh DNS, TCP, and TLS handshakes that significantly slow down un-cached searches. Also, back-to-back benchmarking without delays can trigger server-side rate limits, artificially making a persistent client appear slower.
**Action:** Always reuse a persistent `DDGS()` instance (e.g., via `@st.cache_resource` in Streamlit) instead of using `with DDGS() as ddgs:` for every query.
