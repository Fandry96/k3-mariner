...
## 2025-03-02 - Agent Search Tool Redundancy
**Learning:** The agent frequently re-evaluates the same search query when reflecting, causing redundant and slow network calls. The standard `agent.py` `MarinerSearchTool` misses instance-level caching.
**Action:** Implement a bounded instance-level LRU dictionary cache directly in the tool (`self._cache`) to avoid redundant network calls, making sure to pop and re-insert keys for recency order and bound the cache size to 50 items.
