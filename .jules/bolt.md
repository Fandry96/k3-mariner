...
## 2024-05-25 - Agent Method Caching Memory Leak Anti-Pattern
**Learning:** Applying @functools.lru_cache directly to an instance method causes memory leaks by keeping the long-lived agent instance alive. Unbounded dictionary caches also lead to slow memory growth.
**Action:** Use an instance-level dictionary (self._cache = {}) with a strict bounded size limit (e.g., if len(self._cache) > 50: self._cache.clear()) for caching agent tool outputs.
