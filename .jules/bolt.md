...
## 2024-05-25 - Agent Tool Caching Strategy
**Learning:** Standalone agent tools (like MarinerSearchTool in agent.py) that run outside the Streamlit context cannot benefit from Streamlit's `@st.cache_data`. Without instance-level caching, the agent may make redundant network calls if it re-evaluates the same query.
**Action:** Implement bounded instance-level LRU caching (`self._cache`) in standalone agent tools that perform expensive I/O operations. Ensure the cache size is explicitly bounded to prevent unbounded memory growth.
