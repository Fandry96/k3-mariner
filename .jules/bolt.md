...
## 2025-05-24 - Agent Tool Bounded Caching
**Learning:** Agents can get stuck in loops retrying identical search queries, causing redundant network latency and potential rate limits. Applying `@functools.lru_cache` directly on instance methods leaks memory by caching the instance (`self`), while unbounded dictionaries grow indefinitely.
**Action:** Implement bounded instance-level dictionary caching (`self._cache`) with LRU-style eviction (e.g., `if len(cache) > 50: cache.pop(next(iter(cache)))`) for agent tools, ensuring errors are never cached to avoid poisoning. Apply this consistently across all duplicate tool definitions.
