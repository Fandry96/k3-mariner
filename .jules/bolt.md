...
## 2024-05-28 - Multi-layer caching for agent tools
**Learning:** Streamlit's `@st.cache_data` provides cross-rerun global persistence, but still incurs API overhead within a single agent execution (single rerun).
**Action:** Employ a multi-layer caching strategy: use `@st.cache_data` for global persistence, and an instance-level dictionary (`self._cache = {}`) within the tool class to bypass Streamlit API overhead for repeated queries within the agent loop. Ensure bounded memory growth using LRU-style eviction.
