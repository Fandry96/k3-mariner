...
## 2024-05-25 - Search Tool Caching
**Learning:** Agent tools that perform expensive network operations (like DuckDuckGo search) can be called multiple times with the same query, leading to redundant latency.
**Action:** Implement bounded instance-level LRU caching in tool `forward` methods, ensuring empty results are cached but error states are skipped.
