...
## 2025-03-02 - DuckDuckGo Search Context Manager Anti-Pattern
**Learning:** Instantiating a new `duckduckgo_search.DDGS()` context for every search (`with DDGS() as ddgs:`) destroys the underlying HTTP connection pool after each call. This forces fresh DNS, TCP, and TLS handshakes on every subsequent request, adding significant latency.
**Action:** Always reuse a persistent `DDGS()` instance (e.g., via `@st.cache_resource` in Streamlit or assigning to a class property) instead of using the context manager for individual searches.
