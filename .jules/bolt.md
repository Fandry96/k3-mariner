...
**Learning:** Agent tools can make redundant network calls when an agent repeatedly searches the same query. Unbounded caches can lead to memory bloat over time.
**Action:** Always implement a bounded instance-level dictionary cache (e.g., LRU max 50 items) for agent tool outputs, and only cache successful computations to prevent redundant network calls while protecting memory.
