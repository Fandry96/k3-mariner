...
## 2025-03-12 - Instance-Level Tool Output Caching
**Learning:** Returning early without caching on empty results (like "No results found.") causes repeated identical queries to bypass the cache and continuously trigger expensive network calls.
**Action:** When implementing instance-level LRU caching for tool outputs, ensure that valid 'empty' responses are explicitly cached to prevent redundant network calls, while actual exception or error states are skipped.
