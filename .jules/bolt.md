...
## 2025-05-26 - Instance-Level Agent Caching
**Learning:** For agents executing complex tasks, the same tool (like web search) may be repeatedly called with identical parameters within a single session. Relying purely on global Streamlit caching (e.g., `@st.cache_data`) incurs framework overhead.
**Action:** Implement multi-layer caching: add an instance-level bounded dictionary (`self._cache`) inside agent tool classes. Ensure exceptions/failures are NOT cached, and always enforce a size limit (e.g., LRU via `pop(next(iter(...)))`) to prevent memory bloat.
