...
## 2025-03-05 - Instance-Level Bounded Dictionary Caching
**Learning:** For standalone local agent tool instances (like `MarinerSearchTool`), wrapping the entire API object in `@functools.lru_cache` on class methods causes memory leaks, and reinstantiating connection objects per request drops HTTP connection pooling.
**Action:** Retain a persistent connection object instance (e.g. `self.ddgs = DDGS()`) and implement a bounded instance-level dictionary (`self._cache = {}`) with a size limit (e.g., `if len(self._cache) > 50: self._cache.clear()`) to cache duplicate queries safely and efficiently without leaking memory.
