...
## 2025-03-28 - duckduckgo_search Connection Pool Teardown
**Learning:** Using `with DDGS() as ddgs:` for web searches in a Streamlit app destroys the underlying HTTP connection pool after every use, requiring fresh DNS, TCP, and TLS handshakes on subsequent searches. This significantly slows down un-cached requests.
**Action:** Always reuse a persistent `DDGS()` instance (e.g., via `@st.cache_resource` in Streamlit) to maintain the connection pool across agent executions and Streamlit reruns.
