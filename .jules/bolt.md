...
## 2025-03-01 - Tool Output Caching for Empty Results
**Learning:** Returning early without caching when handling empty tool outputs (e.g., 'No results found.') causes repeated identical queries to bypass the cache and continuously trigger expensive network calls.
**Action:** When implementing caching for tool outputs, ensure that valid 'empty' responses are appropriately formatted and cached. Error/exception states (e.g., 'SEARCH FAILED') should still be skipped and not cached.
