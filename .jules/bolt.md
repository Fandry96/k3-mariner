...
## 2025-03-01 - Caching Tool Outputs Gracefully
**Learning:** When implementing caching for tool outputs (like DuckDuckGo search), empty but valid responses (like "No results found.") must be cached. Early returns bypassing cache for empty lists will cause repeated identically bad queries to continually trigger network requests, degrading performance.
**Action:** Format 'empty' responses correctly as valid string output and store them in the cache just like populated results. Skip caching only on actual error/exception states.
