...
## 2025-03-09 - Agent Tool Call Caching
**Learning:** Agent tool calls within reasoning loops often repeat identical queries when the agent gets stuck or re-evaluates. The `MarinerSearchTool` previously made redundant DuckDuckGo network requests for duplicate queries, causing significant delays and risking rate limits.
**Action:** Always implement internal caching (e.g., `@functools.lru_cache`) on the underlying API calls within agent Tools to memoize results for identical inputs, saving ~1s per redundant request.
