...
## 2025-03-01 - Bounded Caching for Tool Outputs
**Learning:** Using `@functools.lru_cache` directly on instance methods caches `self`, which keeps the instance alive indefinitely and causes memory leaks. Bounded instance-level dictionaries for cache are critical to preventing unbounded memory growth.
**Action:** Implement bounded instance-level dictionaries (e.g., `self._cache = {}` with limit) to safely cache agent tool outputs without causing memory leaks.
