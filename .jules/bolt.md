...
## 2025-05-31 - Multi-Layer Caching for Agent Tools
**Learning:** Using `@st.cache_data` for agent tools provides cross-rerun global persistence but introduces Streamlit API overhead. For repeated queries within a single agent execution, this overhead is measurable and unnecessary.
**Action:** Employ a multi-layer caching strategy: use `@st.cache_data` for global persistence and an instance-level bounded dictionary (`self._cache`) to bypass API overhead during active agent sessions. Do not cache transient failures.
