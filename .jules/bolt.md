...
## 2025-03-05 - Bounded Dictionary Caching
**Learning:** Unbounded instance-level dictionary caches in long-running tools can cause memory bloat and cache thrashing.
**Action:** Always enforce a max size (e.g., 50) using `self._cache.pop(next(iter(self._cache)))` for LRU-style eviction.
