...
## 2025-03-01 - Instance-Level Bounded LRU Cache
**Learning:** Tool-level API calls without caching cause redundant network overhead. Using unbounded dictionaries for caching can cause memory leaks over time.
**Action:** Implement bounded LRU-style dictionary caching for tool outputs, popping/re-inserting on cache hits and enforcing a max size limit.
