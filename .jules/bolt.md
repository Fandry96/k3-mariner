...
## 2024-07-28 - Cache Agent Tool Outputs
**Learning:** The standalone agent performs redundant DuckDuckGo searches for identical queries when Streamlit's cache is not available, resulting in significant network overhead.
**Action:** Implemented a bounded LRU cache using an instance dictionary (`self._cache = {}`) within the `MarinerSearchTool` to avoid repeated expensive network calls. Valid empty responses are cached, but exceptions are skipped.
