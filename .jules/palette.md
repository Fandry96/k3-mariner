...
## 2024-06-08 - Disabling Buttons for Missing Prerequisites
**Learning:** Showing an error message only *after* a user submits a form can be frustrating, especially for prerequisites like an API key. Disabling the submit button dynamically with a helpful tooltip prevents invalid states and improves the user experience.
**Action:** Use `disabled` and `help` parameters on actionable elements when required fields are missing.
