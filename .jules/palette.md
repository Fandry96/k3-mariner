...
## 2024-06-06 - Preemptive Form Validation
**Learning:** Showing an error message only after a user submits an invalid form is frustrating. Streamlit's `disabled` and `help` parameters on `st.form_submit_button` can dynamically prevent invalid submissions.
**Action:** Always use `disabled` and `help` parameters on `st.form_submit_button` to dynamically disable the button and provide an explanatory tooltip when required fields (like API keys) are missing.
