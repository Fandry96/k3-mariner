...
## 2024-05-24 - Streamlit Form Button States
**Learning:** In Streamlit, dynamically disabling an `st.form_submit_button` using inputs from inside the same form permanently locks the button because widgets inside forms do not trigger reruns until submission.
**Action:** When implementing disabled states for form submit buttons, only use variables from widgets located outside the form (e.g., configuration fields) to safely manage the button's interactive state.
