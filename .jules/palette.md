...
## 2024-06-04 - Proactive Form Validation
**Learning:** Users submitting forms only to receive an error about missing prerequisites (like API keys) creates friction. Disabling the submit button dynamically and adding an explanatory tooltip prevents invalid submissions and guides the user.
**Action:** Always use the `disabled` and `help` parameters on Streamlit form submit buttons when required inputs are missing, rather than handling it after submission.
