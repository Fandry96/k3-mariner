...
## 2024-05-24 - Dynamic Form Action Disabled States
**Learning:** Showing an error message after a user attempts an invalid action (like submitting without an API key) is frustrating and reactive. Disabling the action proactively with an explanatory tooltip improves user flow and prevents errors before they occur.
**Action:** Use the `disabled` and `help` parameters on Streamlit buttons to preemptively prevent invalid states instead of relying on post-action validation errors.
