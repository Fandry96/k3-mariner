...
## 2024-06-13 - [Disable Submit Button]
**Learning:** Preventing invalid Streamlit form submissions by dynamically disabling the button and providing an explanatory tooltip when required fields (like API keys) are missing is a significantly better UX than showing an error message after the user clicks submit.
**Action:** Always use the `disabled` and `help` parameters on actionable elements to guide the user proactively instead of reactively handling errors.
