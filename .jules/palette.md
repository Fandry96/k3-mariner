...
## 2024-06-01 - Disable Submit Button on Missing API Key
**Learning:** Using `disabled` and `help` parameters on `st.form_submit_button` provides immediate visual feedback and prevents users from submitting invalid forms. This is much better UX than showing an error message after submission.
**Action:** Always dynamically disable submit buttons and provide explanatory tooltips when required fields are missing in Streamlit forms.
