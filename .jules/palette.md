...
## 2024-06-15 - Disabled States for Invalid Forms
**Learning:** Letting users submit a form only to see an error (like a missing API key) is frustrating. Dynamically disabling the submit button and providing an explanatory tooltip prevents the error state entirely and guides the user.
**Action:** Use `disabled` and `help` parameters on `st.form_submit_button` to prevent invalid form submissions instead of relying solely on post-submission error messages.
