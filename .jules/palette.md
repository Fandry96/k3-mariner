...
## 2024-05-24 - API Key Empty State
**Learning:** Tooltips (`help` parameter) are sometimes missed by users who don't hover. For critical setup inputs (e.g., API keys), unconditionally hiding the onboarding link in a tooltip leads to abandonment when the field is empty.
**Action:** Conditionally display an `st.caption` with a direct link when a critical input is empty, in addition to the `help` tooltip.
