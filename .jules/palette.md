...
## 2024-05-30 - Prevent Invalid Form Submissions
**Learning:** Allowing users to submit a form when a required external credential (like an API Key) is missing leads to frustration and a negative feedback loop (an error shown after action).
**Action:** Dynamically disable the submit button and provide a helpful tooltip explaining why it is disabled, guiding the user to the prerequisite action rather than letting them fail.
