...
## 2024-08-28 - Dynamic Form Validation in Streamlit
**Learning:** Streamlit forms do not trigger reruns on internal widget changes, making real-time validation of internal fields impossible. However, we can dynamically disable the form submit button based on the state of required inputs located outside the form.
**Action:** Use `disabled` and `help` parameters on `st.form_submit_button` tied to external required inputs to prevent invalid submissions and guide the user.
