...
## 2024-05-31 - [Disable Submit Buttons on Missing Prerequisites]
**Learning:** Preventing invalid Streamlit form submissions by dynamically using the `disabled` and `help` parameters on the submit button provides clearer upfront constraints than post-interaction error validation.
**Action:** Always prefer disabling forms with tooltips over allowing the submission to fail with an error state.
