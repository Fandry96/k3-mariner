...
## 2024-05-26 - Form Validation Feedback
**Learning:** Users may accidentally submit empty forms in Streamlit which triggers useless backend operations. Returning early with `st.warning()` provides immediate inline feedback without crashing the app.
**Action:** Always validate required text inputs in Streamlit forms before processing, using inline warnings (e.g., `st.warning`) for clear feedback to prevent empty queries and useless agent runs.
