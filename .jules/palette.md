...
## 2024-07-10 - Dynamic Form Button Disablement
**Learning:** Streamlit forms can frustrate users when they allow submission without required prerequisites (like an API key), leading to post-submit validation errors.
**Action:** Always use the `disabled` and `help` parameters on `st.form_submit_button` to proactively guide users and prevent invalid state submissions, evaluating prerequisite state from outside the form.
